...
start() ->
    io:fwrite("\n\n Supp Buddies!!, Welcome to Twitter Social world!! \n\n"),
    ...
    {ok, ListenSocket} = gen_tcp:listen(1204, [
        ...
        {ok, Data1} ->
            ...
            if
                Type == "register" ->
                    ...
                    io:format("\nSocket:~p\n", [Socket]),
                    io:format("Type: ~p\n", [Type]),
                    io:format("\n~p User want to register account\n", [UserName]),
                    ...
                    if
                        Output == [] ->
                            ...
                            % RESPOND BACK - YES/NO
                            ok = gen_tcp:send(Socket, "User registered"),
                            io:fwrite(
                                "Bad day But i got your back chill dude!!, Key is not in database.\n"
                            );
                        true ->
                            ok = gen_tcp:send(
                                Socket,
                                "Username already exists! Register with a new username"
                            ),
                ...
                Type == "tweet" ->
                    ...
                    io:format("\n ~p tweet is sent: ~p", [UserName, Tweet]),
                    ...
                    ets:insert(
                        Table, {UserName, [{"followers", CurrentFollowers}, {"tweets", NewTweets}]}
                    ),
                    ...
                    ok = gen_tcp:send(Socket, "tweet is processed by server\n"),
                    do_recv(Socket, Table, [UserName], Client_Socket_Mapping);
                Type == "retweet" ->
                    ...
                    io:format("Enter the username for retweeting: ~p\n", [Sub_User]),
                            ...
                            if
                                CheckTweet == true ->
                                    NewTweet = string:concat(
                                        string:concat(string:concat("re:", Sub_User), "->"), Tweet
                                    ),
                                    sendMessage(
                                        Socket,
                    ...
                    ok = gen_tcp:send(Socket, "retweet is processed by server\n"),
                    do_recv(Socket, Table, [UserName], Client_Socket_Mapping);
                Type == "subscribe" ->
                    UserName = binary_to_list(lists:nth(2, Data)),
                    ...
                    if
                        ...
                        true ->
                            ...
                            ets:insert(
                                Table,
                                {Sub_User, [{"followers", NewFollowers}, {"tweets", CurrentTweets}]}
                            ),
                ...
                Type == "query" ->
                    ...
                    if
                        ...
                        Option == "2" ->
                            ...
                            ok = gen_tcp:send(Socket, Tweets);
                        true ->
                            io:fwrite("Subscriber tweet Search\n"),
...
searchAllTweets(Symbol, Table, Key, Word, Found) ->
    ...
    io:format("Enter the word to be searched: ~p~n", [Search]),
    if
        Key == '$end_of_table' ->
...
sendMessage(Socket, Client_Socket_Mapping, Tweet, Subscribers, UserName) ->
    if
        ...
        true ->
            ...
            ok = gen_tcp:send(Client_Socket, ["Received a new tweet!\n", UserName, ":", Tweet]),
            ok = gen_tcp:send(Socket, "Tweet sent!!\n"),
