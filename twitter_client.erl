...
start() ->
    ...
    spawn(twitter_client, get_and_parse_user_input, [Sock, "_"]),
    loop(Sock).
...
get_and_parse_user_input(Sock, UserName) ->
    ...
    if
        ...
        CommandType == "tweet" ->
            if
                UserName == "_" ->
                    io:fwrite("Please register first and proceed to login!\n"),
        ...
        true ->
            io:fwrite("Invalid command!, Please Input the Operation!\n"),
            UserName1 = get_and_parse_user_input(Sock, UserName)
    end,
...
re_tweet(Socket, UserName) ->
    {ok, [Person_UserName]} = io:fread(
        "\nInput the UserName for retweeting: ", "~s\n"
    ),
    ok = gen_tcp:send(Socket, ["query", ",", UserName, ",", "3", ",", Person_UserName]),
    Tweet = io:get_line("\nEnter the tweet that you want to retweet: "),
